...
def analyse(symbol, start, end):
    ...
    df['fundingRate_future'] = df['fundingRate_future'].apply(to_decimal)
    ...
    ### position and trade
    df['position'] = np.where((df['basis'] < 0) & (df['fundingRate_future'] < 0), Decimal('1'), 
                              np.where((df['basis'] > 0) & (df['fundingRate_future'] > 0), Decimal('-1'), 
                                       np.where(df['basis'] == 0, Decimal('0'),
                                                np.nan)))
    ...
    # filtered out the trades occured
...
try:
    ...
    analyse(symbol, f"{start}h", f"{end}h")
    # analyse_candlestick(symbol, f"{start}", "-128h")
